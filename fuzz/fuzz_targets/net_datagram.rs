...
fuzz_target!(|data: &[u8]| {
    ...
    let written = match datagram.body {
        ...
        Body::Join(body) => wire::write_join(&mut again, addressing, &body),
        Body::Roster(body) => wire::write_roster(&mut again, addressing, &body)
            .expect("what the reader accepted, the writer must accept"),
        Body::Start(body) => wire::write_start(&mut again, addressing, &body),
